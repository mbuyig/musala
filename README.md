# Career Plan Dashboard

**Roadmap period:** 1 June 2026 - 31 May 2028  
**Target positioning:** Principal Cloud Security, FinOps & Platform Governance Consultant  
**Current phase:** June-July 2026 - AWS Solutions Architect Associate

This is the starting point for using the career plan. Keep it short. Open the
focused files below when you need more detail, and use the
[full roadmap](roadmap/full-roadmap.md) as the reference document.

## Current Focus

| Area | This Phase |
|---|---|
| Certification | AWS Solutions Architect Associate |
| Portfolio | Multi-account AWS landing zone baseline |
| Evidence | Terraform VPC, IAM, logging and shared-services baseline |
| Communication | Architecture diagram and initial cost assumptions |
| Timeframe | June-July 2026 |

Open [tracking/now.md](tracking/now.md) for the active checklist and weekly plan.

## Next Milestone

By the end of July 2026:

- [ ] AWS SAA completed or booked
- [ ] Landing zone baseline started
- [ ] Architecture diagram created
- [ ] Initial cost assumptions documented

## Quick Links

| File | Use It For | Review Frequency |
|---|---|---|
| [Current focus](tracking/now.md) | This week's actions and notes | Weekly |
| [Execution plan](roadmap/execution-plan.md) | Phase-by-phase navigation | Monthly |
| [Commercial playbook](roadmap/commercial-playbook.md) | Offers, positioning and market activity | Monthly |
| [Market log](tracking/market-log.md) | Roles, recruiters, rates and keyword evidence | As opportunities appear |
| [Quarterly reviews](tracking/quarterly-reviews.md) | Decisions and course corrections | Quarterly |
| [Full roadmap](roadmap/full-roadmap.md) | Detailed source of truth | When deeper context is needed |

## Working Rhythm

1. Open [tracking/now.md](tracking/now.md) at the start of the week.
2. Pick up to three concrete priorities.
3. Update the checklist during the week.
4. Add market evidence to [tracking/market-log.md](tracking/market-log.md).
5. Review progress at the end of the week.
6. Update this dashboard only when the current phase changes.

## File Map

```text
.
├── README.md
├── roadmap/
│   ├── full-roadmap.md
│   ├── execution-plan.md
│   └── commercial-playbook.md
└── tracking/
    ├── now.md
    ├── market-log.md
    └── quarterly-reviews.md
```

## Maintenance Rule

Keep the detailed strategy in [roadmap/full-roadmap.md](roadmap/full-roadmap.md).
Keep the dashboard and tracking files concise. When the plan changes, update the
full roadmap first, then update only the affected summary or checklist.
